...
import {
    ...
    PlayCircle, RotateCw, FileCheck2, ScrollText, Calendar, Percent, Trash2, Receipt, Sparkles, CreditCard
} from "lucide-react";
...
type Target = {
    ...
    stripe_connection: { id: string; status: string } | null;
};
...
export function DevModePanel({ target }: { target: Target }) {
    ...
    const hasShopify = !!target.shopify_domain;
    const hasStripe = !!target.stripe_connection;
    const noSource = !hasShopify && !hasStripe;
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 font-semibold mt-1">
                                ...
                                {hasStripe && !hasShopify && <span className="text-slate-600"> · Stripe ({target.stripe_connection!.status})</span>}
                            </p>
                        </div>
                    </div>
                </div>
                {noSource && (
                    <div className="glass rounded-2xl p-5 border border-amber-500/30 bg-amber-500/5 flex items-center gap-3 text-amber-300 text-sm font-bold">
                        ...
                        Esta conta não tem origem configurada (nem Shopify nem Stripe). Acções Dev Mode indisponíveis.
                    </div>
                )}
            </div>
            ...
            {/* Source-agnostic cards: tax override + pending VIES + notify emails
                work for both Shopify and Stripe (queries pivot on user_id). */}
            {(hasShopify || hasStripe) && (
                <>
            ...
            {hasStripe && (
                <>
                    <StripeBackfillCard targetUserId={target.id} notifyEmails={notifyEmails} />
                    <StripeReemitCard targetUserId={target.id} notifyEmails={notifyEmails} />
                    <StripeCancelCard targetUserId={target.id} notifyEmails={notifyEmails} />
                    <StripeFinalizeDraftsCard targetUserId={target.id} notifyEmails={notifyEmails} />
                </>
            )}

            {(hasShopify || hasStripe) && <LogsCard targetUserId={target.id} />}
        </div>
    );
}
...
// ─── Stripe Dev Mode cards ───────────────────────────────────────────────────
// Mirror the Shopify Backfill/Reemit/Cancel/FinalizeDrafts cards but keyed by
// Stripe identifiers (payment_intent / charge / checkout session) instead of
// Shopify order_number. Backend: /api/admin/dev-mode/stripe/*.

function StripeBackfillCard({ targetUserId, notifyEmails }: { targetUserId: string; notifyEmails: string[] }) {
    const [mode, setMode] = useState<"date_range" | "since_last">("date_range");
    const [from, setFrom] = useState("");
    const [to, setTo] = useState(new Date().toISOString().slice(0, 10));
    const [dryRun, setDryRun] = useState(true);
    ...
    const run = async () => {
        setLoading(true); setResult(null);
        try {
            const res = await fetch("/api/admin/dev-mode/stripe/backfill", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    targetUserId, dry_run: dryRun, reason,
                    notify_emails: notifyEmails,
                    ...(mode === "since_last"
                        ? { since_last_processed: true, to: new Date(to + "T23:59:59Z").toISOString() }
                        : { from: from ? new Date(from + "T00:00:00Z").toISOString() : undefined, to: to ? new Date(to + "T23:59:59Z").toISOString() : undefined }),
    ...
    return (
        <Section icon={<CreditCard className="w-5 h-5 text-emerald-400" />} title="Backfill Stripe → IX" desc="Lista PaymentIntents Stripe (status=succeeded) no intervalo escolhido e emite faturas para os que ainda não existem em InvoiceXpress.">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex gap-2 md:col-span-2">
                    {(["date_range", "since_last"] as const).map(m => (
                        <button key={m} onClick={() => setMode(m)}
                            className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${mode === m ? "bg-sky-500/20 text-sky-300 border-sky-500/40" : "bg-slate-900/50 text-slate-500 border-slate-800 hover:text-slate-300"}`}>
                            {m === "date_range" ? "Intervalo de Datas" : "Desde Última PI Processada"}
                        </button>
                    ))}
                </div>
                ...
                <label className="md:col-span-2 flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={dryRun} onChange={e => setDryRun(e.target.checked)} className="accent-amber-500 w-4 h-4" />
                    <span className="text-xs font-bold text-slate-300">
                        Dry-run (lista PIs que seriam processadas, não escreve em IX)
                    </span>
                </label>
...
function StripeReemitCard({ targetUserId, notifyEmails }: { targetUserId: string; notifyEmails: string[] }) {
    ...
    const run = async () => {
        ...
        setLoading(true); setResult(null);
        try {
            const res = await fetch("/api/admin/dev-mode/stripe/reemit", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ targetUserId, stripe_id: stripeId.trim(), force, reason, notify_emails: notifyEmails }),
            });
            setResult(await res.json());
        } catch (e: any) { setResult({ error: String(e) }); }
        finally { setLoading(false); }
    ...
    return (
        <Section icon={<RotateCw className="w-5 h-5 text-amber-400" />} title="Re-emitir Fatura Stripe Única" desc="Força criação de fatura a partir de um identificador Stripe (pi_… / ch_… / cs_… / in_…). Útil para corrigir falhas pontuais.">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <label className="flex flex-col gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-500 md:col-span-1">
                    Stripe id
                    <input type="text" value={stripeId} onChange={e => setStripeId(e.target.value)} placeholder="pi_3Ab... ou ch_3Ab... ou cs_test_..."
                        className="bg-slate-900/50 border border-slate-800 rounded-xl px-3 py-2 text-sm font-medium text-white" />
                </label>
                <label className="md:col-span-2 flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={force} onChange={e => setForce(e.target.checked)} className="accent-rose-500 w-4 h-4" />
                    <span className="text-xs font-bold text-slate-300">
                        Forçar (apaga registo prévio em processed_orders, ignora dedup IX)
                    </span>
                </label>
                <textarea placeholder="Motivo..." value={reason} onChange={e => setReason(e.target.value)} rows={2}
                    className="md:col-span-3 bg-slate-900/50 border border-slate-800 rounded-xl px-3 py-2 text-sm font-medium text-white resize-none" />
            </div>
            <button onClick={run} disabled={loading || !stripeId.trim()}
                className="w-full bg-white text-black py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-amber-500 hover:text-white transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <RotateCw className="w-4 h-4" />}
                Re-emitir
            </button>
...
function StripeCancelCard({ targetUserId, notifyEmails }: { targetUserId: string; notifyEmails: string[] }) {
    const [stripeId, setStripeId] = useState("");
    const [mode, setMode] = useState<"delete_draft" | "credit_note">("delete_draft");
    const [reason, setReason] = useState("");
    const [loading, setLoading] = useState(false);
    ...
    const run = async () => {
        ...
        try {
            const path = mode === "delete_draft" ? "delete-draft" : "issue-credit-note";
            const res = await fetch(`/api/admin/dev-mode/stripe/${path}`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ targetUserId, stripe_id: stripeId.trim(), reason, notify_emails: notifyEmails }),
            });
            setResult(await res.json());
        } catch (e: any) { setResult({ error: String(e) }); }
        finally { setLoading(false); }
    ...
    return (
        <Section
            icon={isDelete ? <Trash2 className="w-5 h-5 text-red-400" /> : <Receipt className="w-5 h-5 text-orange-400" />}
            title="Cancelar / Anular Fatura Stripe"
            desc="Elimina draft (se ainda não finalizada) ou emite nota de crédito (se já finalizada), a partir do identificador Stripe usado quando a fatura foi criada."
        >
            <div className="flex gap-2">
                {(["delete_draft", "credit_note"] as const).map(m => (
                    <button key={m} onClick={() => setMode(m)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <label className="flex flex-col gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-500 md:col-span-1">
                    Stripe id
                    <input type="text" value={stripeId} onChange={e => setStripeId(e.target.value)} placeholder="pi_3Ab... ou ch_..."
                        className="bg-slate-900/50 border border-slate-800 rounded-xl px-3 py-2 text-sm font-medium text-white" />
                </label>
                <textarea placeholder="Motivo (recomendado)..." value={reason} onChange={e => setReason(e.target.value)} rows={2}
                    className="md:col-span-2 bg-slate-900/50 border border-slate-800 rounded-xl px-3 py-2 text-sm font-medium text-white resize-none" />
            ...
            {confirm ? (
                <div className="flex items-center gap-3 bg-red-500/5 border border-red-500/20 rounded-2xl p-4">
                    <AlertCircle className="w-4 h-4 text-red-400" />
                    <span className="text-xs font-bold text-red-300 flex-1">
                        Confirmar: {isDelete ? `eliminar draft do ${stripeId}` : `emitir nota de crédito para ${stripeId}`}? Acção irreversível em IX.
                    </span>
                    <button onClick={run} disabled={loading}
                        className={`px-5 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest ${isDelete ? "bg-red-500 hover:bg-red-600" : "bg-orange-500 hover:bg-orange-600"} text-white disabled:opacity-50 flex items-center gap-2`}>
                        {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle2 className="w-3 h-3" />}
                ...
                <button onClick={() => setConfirm(true)} disabled={loading || !stripeId.trim()}
                    className={`w-full py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all disabled:opacity-50 flex items-center justify-center gap-2 ${isDelete ? "bg-white text-black hover:bg-red-500 hover:text-white" : "bg-white text-black hover:bg-orange-500 hover:text-white"}`}>
                    {isDelete ? <Trash2 className="w-4 h-4" /> : <Receipt className="w-4 h-4" />}
                    {isDelete ? "Eliminar Draft" : "Emitir Nota de Crédito"}
                </button>
...
function StripeFinalizeDraftsCard({ targetUserId, notifyEmails }: { targetUserId: string; notifyEmails: string[] }) {
    const [limit, setLimit] = useState("100");
    const [dryRun, setDryRun] = useState(true);
    const [reason, setReason] = useState("");
    const [dateStrategy, setDateStrategy] = useState<"today" | "closest_available">("closest_available");
    const [filterMode, setFilterMode] = useState<"all" | "date_range">("all");
    const [fromDate, setFromDate] = useState("");
    const [toDate, setToDate] = useState("");
    const [loading, setLoading] = useState(false);
    const [result, setResult] = useState<JobResult | null>(null);
    ...
    const run = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/admin/dev-mode/stripe/finalize-drafts", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload),
            });
    ...
    return (
        <Section icon={<FileCheck2 className="w-5 h-5 text-violet-400" />} title="Finalizar Rascunhos Stripe em Massa" desc="Itera processed_orders (source=stripe, oldest→newest), identifica faturas em draft no IX e finaliza com estratégia de data escolhida.">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-3 flex flex-col gap-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Estratégia de data ao finalizar</span>
                    <div className="flex gap-2">
                ...
                <div className="md:col-span-3 flex flex-col gap-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Filtro</span>
                    <div className="flex gap-2">
                        {([
                            { id: "all", label: "Todos" },
                            { id: "date_range", label: "Range de datas (created_at)" },
                        ] as const).map(opt => (
                            <button key={opt.id} type="button" onClick={() => setFilterMode(opt.id)}
                                className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${filterMode === opt.id ? "bg-violet-500/20 text-violet-300 border-violet-500/40" : "bg-slate-900/50 text-slate-500 border-slate-800 hover:text-slate-300"}`}>
                                {opt.label}
