...
The full set of `IncidentKind` values is the `IncidentKind` union at the top of `src/services/email-templates.ts`; every kind needs a `case` in `renderIncidentTemplate` or `tsc` fails. `queue_retry_exhausted` means a webhook was retried until it gave up (the Shopify consumer's give-up and the dead-letter queue) and nothing else. Conditions that are not retries have their own kinds: `reconcile_sweep_stale`, `auto_heal_failed`, `document_log_write_lost`, `queue_enqueue_failed`, `lodgify_bookings_uninvoiced`.
