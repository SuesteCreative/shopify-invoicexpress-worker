...
app.get("/admin/logs", async (c) => {
  ...
  const appStorage = new AppStorage(c.env, shop ?? undefined, userId ?? undefined);
  try {
    if (type === "errors") {
      return c.json({ entries: userId ? await appStorage.getLogsByUser(userId, limit, "errors") : await appStorage.getLogs(limit, "errors") });
    }
    if (type === "all") {
      return c.json({ entries: userId ? await appStorage.getLogsByUser(userId, limit, "all") : await appStorage.getLogs(limit, "all") });
    }
    if (type === "webhooks") {
      return c.json({ entries: userId ? await appStorage.getWebhookEventsByUser(userId, limit) : await appStorage.getWebhookEvents(limit) });
    }
    return c.json({ entries: userId ? await appStorage.getDevJobsByUser(userId, limit) : await appStorage.getDevJobs(limit) });
  } catch (e) {
    return c.json({ error: String(e) }, 500);
  }
})
...
// Admin: get/set per-account notify emails. Accepts either ?shop= or ?user_id=
// so Stripe-only users (no shopify_domain) can use the same endpoint.
...
app.put("/admin/notify-emails", async (c) => {
  const unauth = requireAdmin(c);
  if (unauth) return unauth;
  const body = await c.req.json<{ shop?: string; user_id?: string; emails: string[] }>();
  if (!body.shop && !body.user_id) return c.json({ error: "Missing shop or user_id" }, 400);
  const emails = (body.emails ?? []).filter(e => typeof e === "string" && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e));
  const appStorage = new AppStorage(c.env, body.shop ?? undefined, body.user_id ?? undefined);
...
// Admin: get/set per-account tax override. Accepts either ?shop= or ?user_id=.
...
// Admin: list pending reverse-charge rows (pending status only). Accepts
// ?shop= (Shopify path) or ?user_id= (Stripe — pivots to user_id scoping).
app.get("/admin/pending-reverse-charge", async (c) => {
  ...
  if (!shop && !userId) return c.json({ error: "Missing shop or user_id" }, 400);
  const sql = userId
    ? "SELECT id, order_id, vat_id, country_code, attempts, status, next_retry_at, last_error, incident_id, created_at, updated_at FROM pending_reverse_charge WHERE user_id = ? AND status = 'pending' ORDER BY created_at DESC LIMIT 100"
  ...
  const result = await c.env.DB.prepare(sql).bind(userId ?? shop!).all();
...
// ────────────────────────────────────────────────────────────────────────────
// Stripe Dev Mode admin endpoints. All key off `user_id` since Stripe-only
// users have no shopify_domain. They load the IRequestConfig via the user-id
// path on AppStorage.
// ────────────────────────────────────────────────────────────────────────────

async function loadConfigForUser(c: Context<{ Bindings: Env }>, userId: string) {
  const storage = new AppStorage(c.env, undefined, userId);
  const config = await storage.loadConfigByUser(userId);
  return config;
}
...
app.post("/admin/stripe/backfill", async (c) => {
  ...
  try {
    ...
    console.error("[Rioko][Stripe Backfill] error:", e);
    return c.json({ error: String(e) }, 500);
  }
})
