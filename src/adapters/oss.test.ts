...
function ctx(over: { engine?: boolean; gross?: boolean; dest?: any; overrides?: any; mappings?: any } = {}): AdapterCtx {
  return {
    apiKey: "k",
    config: { vat_included: over.gross === false ? 0 : 1 } as any,
...
describe("the money does not move when the rate does", () => {
  it("re-rates a French sale from 0% to 20% and still totals what was paid", () => {
...
describe("what the engine refuses to touch", () => {
  ...
  it("never overwrites a domestic reduced rate", () => {
  ...
  it("yields to a mapped Moloni product, whose own tax rule the destination applies", () => {
...
describe("when the rate cannot be applied, it holds instead of guessing", () => {
  it("refuses to add VAT on top of a net price, and says why", () => {
  ...
  it("refuses a foreign rate Vendus would silently mis-declare", () => {
...
describe("the rate table and the country rule", () => {
  ...
  it("follows the shipping address, then billing — the opposite of the client block", () => {
  ...
  it("answers null for a country it has no rate for, rather than guessing", () => {
