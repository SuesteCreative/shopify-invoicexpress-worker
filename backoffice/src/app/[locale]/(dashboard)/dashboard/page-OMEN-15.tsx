...
import { Activity, ShieldCheck, ClipboardList, Settings2, BookOpen, Plus, Store, CheckCircle2, Zap, ArrowRight, TrendingUp, Package, AlertCircle, CreditCard, Webhook } from "lucide-react";
...
export default function WelcomeDashboard() {
  ...
  const [stripeIxStatus, setStripeIxStatus] = useState<any>(null);
  const [isRegistered, setIsRegistered] = useState<boolean | null>(null);

  useEffect(() => {
    Promise.all([
      fetch("/api/integrations").then(res => res.json()).catch(() => ({})),
      fetch("/api/integrations/stripe-source").then(res => res.ok ? res.json() : {}).catch(() => ({}))
    ]).then(([data, stripeData]: any) => {
      ...
      const conn = stripeData?.connection;
      if (conn?.source_config?.stripe_account_id) {
        const stripeOk = !!conn.source_config.stripe_account_id;
        const webhookOk = !!conn.source_config.has_webhook_secret;
        const ixOk = data.ix_authorized === 1;
        setStripeIxStatus({
          id: "stripe-ix",
          status: conn.status,
          stripeAuthorized: stripeOk,
          webhookInstalled: webhookOk,
          ixAuthorized: ixOk,
          stripeAccountId: conn.source_config.stripe_account_id,
          isAllComplete: conn.status === "active" && stripeOk && webhookOk && ixOk
        });
      }
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-1000 slide-in-from-bottom-4">
      ...
      <div className="space-y-6">
        ...
        {(integrationStatus || stripeIxStatus) ? (
          <div className="grid gap-6">
            ...
            {stripeIxStatus && (
            <Link
              href="/integrations/stripe-ix"
              className="glass p-8 rounded-[2.5rem] border-slate-800/40 hover:border-indigo-500/30 transition-all flex flex-col md:flex-row items-center justify-between gap-8 group relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-8 opacity-0 group-hover:opacity-5 transition-all">
                <Settings2 className="w-32 h-32 text-indigo-400" />
              </div>
              <div className="flex items-center gap-8 relative z-10 w-full md:w-auto">
                <div className="w-20 h-20 rounded-[1.8rem] bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20 shrink-0">
                  <CreditCard className="w-10 h-10 text-indigo-400" />
                </div>
                <div className="space-y-1">
                  <h3 className="text-2xl font-black tracking-tight group-hover:text-indigo-400 transition-colors">Stripe + InvoiceXpress</h3>
                  <div className="flex items-center gap-3">
                    <span className={cn(
                      "px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest border",
                      stripeIxStatus.isAllComplete ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-amber-500/10 text-amber-500 border-amber-500/20"
                    )}>
                      {stripeIxStatus.isAllComplete ? "Ativa e Autorizada" : (stripeIxStatus.status === "active" ? "Ativa · Verificar" : "Configuração Pendente")}
                    </span>
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest hidden sm:inline font-mono">{stripeIxStatus.stripeAccountId}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6 relative z-10 ml-auto md:ml-0">
                <div className="hidden xl:flex items-center -space-x-2">
                  <div className={cn("w-8 h-8 rounded-full border-4 border-slate-950 flex items-center justify-center", stripeIxStatus.stripeAuthorized ? "bg-indigo-500/20 text-indigo-400" : "bg-slate-800 text-slate-600")}><CreditCard className="w-3 h-3" /></div>
                  <div className={cn("w-8 h-8 rounded-full border-4 border-slate-950 flex items-center justify-center", stripeIxStatus.webhookInstalled ? "bg-violet-500/20 text-violet-400" : "bg-slate-800 text-slate-600")}><Webhook className="w-3 h-3" /></div>
                  <div className={cn("w-8 h-8 rounded-full border-4 border-slate-950 flex items-center justify-center", stripeIxStatus.ixAuthorized ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-800 text-slate-600")}><ClipboardList className="w-3 h-3" /></div>
                </div>
                <ArrowRight className="w-6 h-6 text-slate-700 group-hover:text-indigo-400 group-hover:translate-x-2 transition-all" />
              </div>
            </Link>
            )}
          </div>
