...
export default function StripeConnectMoloniIntegration() {
    ...
    // Its own pair's price. This card borrowed Stripe→Moloni's price strings
    // through `tShared`, so it advertised that pair's legacy 5 €/50 € while its
    // own checkout charged this pair's price.
    const prices = useBillingPrice("stripe_connect:moloni");
    ...
    useEffect(() => {
        ...
        fetch("/api/billing/subscription?connection_key=stripe_connect:moloni").then(r => r.json()).then(setSub).catch(() => setSub(null));
