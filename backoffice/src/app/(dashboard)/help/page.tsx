        ...
        <div className="rounded-2xl overflow-hidden border border-hairline/60 bg-surface-2/40 cursor-zoom-in group relative" onClick={() => onZoom(src)}>
            <div className="relative w-full aspect-video bg-surface transition-transform duration-500 group-hover:scale-[1.02]">
                ...
                <div className="absolute bottom-3 left-3 right-3 bg-black/70 backdrop-blur-sm rounded-xl px-3 py-2 flex items-center gap-2">
                    <span className="text-soon text-[10px] font-black uppercase tracking-widest">Visualização</span>
...
function InfoBox({ children }: { children: React.ReactNode }) {
    return (
        <div className="bg-[rgba(2,141,196,0.05)] border border-[rgba(2,141,196,0.20)] rounded-2xl p-4 flex items-start gap-3">
            ...
            <p className="text-accent text-sm leading-relaxed">{children}</p>
        </div>
    );
}
...
function WarningBox({ children }: { children: React.ReactNode }) {
    return (
        <div className="bg-[rgba(245,158,11,0.05)] border border-[rgba(245,158,11,0.20)] rounded-2xl p-4">
            <p className="text-soon text-sm leading-relaxed">{children}</p>
        </div>
    );
}
    ...
    return (
        <div className="glass rounded-[2rem] p-5 border-hairline space-y-5">
            <div className="space-y-3">
                <GroupHeader label="Plataforma de Pagamento" gradientFrom="from-[rgba(94,234,212,0.30)]" gradientVia="via-[rgba(2,141,196,0.30)]" />
            ...
            <div className="space-y-3">
                <GroupHeader label="Software de Faturação" gradientFrom="from-[rgba(2,141,196,0.30)]" gradientVia="via-[rgba(245,158,11,0.30)]" />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {invoicing.map(p => (
                        <PlatformTabButton key={p.id} p={p} active={tab === p.id} onClick={() => onChange(p.id)} />
                    ))}
        ...
        <div className="space-y-6">
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="shopify-domain" icon={<Store className="w-5 h-5" />} title="Domínio da Loja Shopify" step="Credencial 1 de 3" accent="emerald">
                    ...
                    <Steps accent="emerald" items={[
                        'Acede ao painel da loja em <code class="bg-surface-2 text-accent-hot px-1.5 py-0.5 rounded text-xs">admin.shopify.com</code>.',
                        ...
                        'Copia o valor (sem o prefixo <code class="bg-surface-2 text-accent-hot px-1.5 py-0.5 rounded text-xs">https://</code>) e cola-o no campo Domínio do Rioko.',
                    ]} />
            ...
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="shopify-token" icon={<Key className="w-5 h-5" />} title="Access Token" step="Credencial 2 de 3" accent="emerald">
                    ...
                    <InfoBox>
                        <strong>Encomendas com mais de 60 dias</strong> — Por defeito, a Admin API só devolve encomendas recentes. Para reemitir faturas antigas em massa, o token tem de incluir o scope <code className="bg-surface-2 px-1 rounded">read_all_orders</code>. Se vires <em>"Order #XXXX not found"</em> para uma encomenda antiga, pede-nos para regenerar o token com esse scope.
                    </InfoBox>
                </Section>
            </div>
            ...
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="shopify-webhook" icon={<Webhook className="w-5 h-5" />} title="Webhook Signing Secret" step="Credencial 3 de 3" accent="emerald">
                    ...
                    <InfoBox>
                        O Rioko instala automaticamente os webhooks <code className="bg-surface-2 px-1 rounded">orders/paid</code>, <code className="bg-surface-2 px-1 rounded">refunds/create</code> e <code className="bg-surface-2 px-1 rounded">orders/updated</code> quando carregas em "Ativar". Se o token não tiver permissão de webhooks, o Rioko mostra um botão para instalar manualmente.
                    </InfoBox>
                </Section>
            </div>
        ...
        <div className="space-y-6">
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="stripe-account-id" icon={<CreditCard className="w-5 h-5" />} title="Stripe Account ID" step="Credencial 1 de 3" accent="violet">
                    <InfoBox>
                        O Account ID identifica a tua conta Stripe (formato <code className="bg-surface-2 px-1 rounded">acct_XXXXXXXXXXXX</code>). O Rioko usa-o para garantir que cada webhook recebido pertence à tua conta.
                    </InfoBox>
                    <Steps accent="violet" items={[
                        'Inicia sessão em <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">dashboard.stripe.com</code>.',
                        'No canto superior esquerdo, abre o seletor de conta (mostra o nome da empresa).',
                        'O Account ID aparece por baixo do nome, começando por <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">acct_</code>.',
            ...
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="stripe-restricted-key" icon={<Key className="w-5 h-5" />} title="Restricted Key" step="Credencial 2 de 3" accent="violet">
                    ...
                    <Steps accent="violet" items={[
                        ...
                        'Dá-lhe o nome <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">Rioko</code>.',
                        ...
                        'Carrega em <strong>"Create key"</strong>, copia o valor que começa por <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">rk_live_</code> (ou <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">rk_test_</code> em ambiente de testes) e cola no Rioko.',
                    ]} />
                    <WarningBox>
            ...
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="stripe-webhook" icon={<Webhook className="w-5 h-5" />} title="Webhook + Signing Secret" step="Credencial 3 de 3" accent="violet">
                    ...
                    <Steps accent="violet" items={[
                        ...
                        '<strong>URL</strong>: <div class="mt-2 flex items-center gap-2"><code class="bg-surface-2 text-accent px-1.5 py-1 rounded text-[10px] break-all flex-1">https://shopify-invoicexpress-worker.pedrotovarporto.workers.dev/webhooks/stripe</code></div>',
                        '<strong>Events</strong>: selecciona <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">payment_intent.succeeded</code>, <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">charge.succeeded</code> e <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">charge.refunded</code>.',
                        'Confirma criação. Na página do endpoint, carrega em <strong>"Reveal"</strong> sobre o <strong>Signing secret</strong>.',
                        'Copia o valor que começa por <code class="bg-surface-2 text-accent px-1.5 py-0.5 rounded text-xs">whsec_</code> e cola no campo Webhook Secret do Rioko.',
                    ]} />
                    <WarningBox>
        ...
        <div className="space-y-6">
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="ix-account" icon={<FileText className="w-5 h-5" />} title="Nome da Conta" step="Credencial 1 de 2" accent="sky">
                    <InfoBox>
                        O nome da conta InvoiceXpress é o subdomínio da tua conta (ex: <code className="bg-surface-2 px-1 rounded">nome-da-empresa</code> em <code className="bg-surface-2 px-1 rounded">nome-da-empresa.app.invoicexpress.com</code>).
                    </InfoBox>
                ...
                <Section id="ix-environment" icon={<Globe className="w-5 h-5" />} title="Ambiente — Produção vs Sandbox" accent="sky">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="bg-[rgba(94,234,212,0.05)] border border-[rgba(94,234,212,0.20)] rounded-2xl p-5">
                            <div className="text-accent-hot font-black text-sm mb-2">✅ Produção</div>
                            <p className="text-fg-60 text-sm">Faturas reais, com valor fiscal e comunicadas à AT. Usa quando a loja já está ao vivo.</p>
                        </div>
                        <div className="bg-[rgba(245,158,11,0.05)] border border-[rgba(245,158,11,0.20)] rounded-2xl p-5">
                            <div className="text-soon font-black text-sm mb-2">🧪 Sandbox</div>
                            <p className="text-fg-60 text-sm">Ambiente de testes, sem valor fiscal. Apenas para validação técnica antes do go-live.</p>
                        </div>
                    </div>
                </Section>
            </div>
        ...
        <div className="space-y-6">
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="moloni-dev-account" icon={<Key className="w-5 h-5" />} title="Conta de Programador" step="Credencial 1 de 3" accent="amber">
                    ...
                    <Steps accent="amber" items={[
                        'Acede a <code class="bg-surface-2 text-soon px-1.5 py-0.5 rounded text-xs">www.moloni.pt/dev</code>.',
            ...
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="moloni-app" icon={<Settings2 className="w-5 h-5" />} title="Criar Aplicação" step="Credencial 2 de 3" accent="amber">
                    ...
                    <Steps accent="amber" items={[
                        ...
                        '<strong>Nome</strong>: <code class="bg-surface-2 text-soon px-1.5 py-0.5 rounded text-xs">Rioko</code>.',
            ...
            <div className="glass rounded-[2rem] p-8 border-hairline space-y-6">
                <Section id="moloni-credentials" icon={<Globe className="w-5 h-5" />} title="Credenciais da Conta Principal" step="Credencial 3 de 3" accent="amber">
                    ...
                    <Steps accent="amber" items={[
                        '<strong>Username</strong>: o email com que entras em <code class="bg-surface-2 text-soon px-1.5 py-0.5 rounded text-xs">moloni.pt</code> (não o do developer).',
                    ...
                    <WarningBox>
                        ⚠️ <strong>Recomendado:</strong> cria um utilizador dedicado na Moloni para o Rioko (ex: <code className="bg-surface-2 px-1 rounded">rioko@nome-empresa.pt</code>) com permissões só de <em>faturação</em>. Assim podes revogar a integração sem mexer na tua conta pessoal.
                    </WarningBox>
                </Section>
            </div>
    ...
    return (
        <div className="glass rounded-[2.5rem] p-8 lg:p-10 border-hairline space-y-6">
            <div className="flex items-start gap-4">
                ...
                <div>
                    <div className="text-[10px] font-black text-fg-40 uppercase tracking-[0.2em] mb-1">Perguntas Frequentes</div>
            ...
            <div className="glass rounded-[2rem] p-10 border-hairline">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p className="text-fg-60 font-semibold mt-1">Rioko 2.0 · Como obter as credenciais de cada plataforma</p>
                    </div>
                </div>
