...
export default function StripeIXIntegration() {
    ...
    if (!STRIPE_ENABLED) {
        return (
            <div className="max-w-3xl mx-auto py-24 space-y-8 animate-in fade-in duration-700">
                <Link href="/integrations" className="text-[10px] font-black text-accent-ink uppercase tracking-widest hover:text-fg transition-colors flex items-center gap-2">
                    <ArrowLeft className="w-3 h-3" /> {t("backToIntegrations")}
                </Link>
                <div className="glass rounded-[2.5rem] p-6 sm:p-12 border-soon/20 bg-soon/4 flex flex-col items-center text-center gap-6">
                    <div className="w-20 h-20 rounded-2xl bg-soon/10 ring-1 ring-soon/20 flex items-center justify-center">
    ...
    return (
        <div className="space-y-12 animate-in fade-in duration-1000 slide-in-from-bottom-4">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                ...
                <div className="flex items-center gap-5 glass px-5 py-3 rounded-2xl border-hairline">
                    <div className="flex -space-x-2.5">
                        <div className={cn("h-9 w-9 rounded-full ring-4 ring-surface flex items-center justify-center border", hasStripeSaved ? "bg-accent/10 border-accent/30" : "bg-surface-2 border-hairline")}>
                            <CreditCard className={cn("w-4 h-4", hasStripeSaved ? "text-accent-ink" : "text-fg-40")} />
                        </div>
                        <div className={cn("h-9 w-9 rounded-full ring-4 ring-surface flex items-center justify-center border", hasWebhookSaved ? "bg-accent/10 border-accent/30" : "bg-surface-2 border-hairline")}>
                            <Webhook className={cn("w-4 h-4", hasWebhookSaved ? "text-accent-ink" : "text-fg-40")} />
                        </div>
            ...
            <SubscriptionCard onSuccess={stripeSuccess} source="stripe-ix" connectionKey="stripe:invoicexpress" />
                                        ...
                                        {s.kind === "stripe" && (
                                            <div className="grid md:grid-cols-2 gap-8">
                                                <div className="md:col-span-2 flex items-start gap-4 bg-accent/5 border border-accent/20 rounded-2xl px-6 py-4">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-bold text-accent-ink">{t("restrictedKeyHowTitle")}</p>
                                                        <ol className="text-[11px] text-fg-60 mt-2 leading-relaxed list-decimal pl-4 space-y-1">
                                                            <li dangerouslySetInnerHTML={{ __html: t("restrictedKeyStep1").replace(/<span>/g, '<span class="text-accent-ink font-semibold">') }} />
                                                            <li dangerouslySetInnerHTML={{ __html: t("restrictedKeyStep2").replace(/<code>/g, '<code class="bg-surface-2 px-1 rounded text-accent-ink">') }} />
                                                            <li dangerouslySetInnerHTML={{ __html: t("restrictedKeyStep3").replace(/<code>/g, '<code class="bg-surface-2 px-1 rounded text-accent-ink">') }} />
                                                ...
                                                <div className="space-y-3">
                                                    <div className="flex items-center justify-between ml-1">
                                                        ...
                                                        <a href="/help#stripe-account-id" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] font-black text-fg-40 uppercase tracking-widest hover:text-accent-ink transition-colors"><BookOpen className="w-3 h-3" />{t("whereFind")}</a>
                                                    </div>
                                                ...
                                                <div className="space-y-3">
                                                    <div className="flex items-center justify-between ml-1">
                                                        ...
                                                        <a href="/help#stripe-restricted-key" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] font-black text-fg-40 uppercase tracking-widest hover:text-accent-ink transition-colors"><BookOpen className="w-3 h-3" />{t("whatIs")}</a>
                                                    </div>
                                                    ...
                                                    <p className="text-[10px] text-fg-40 font-medium mt-2 ml-1 uppercase tracking-wider" dangerouslySetInnerHTML={{ __html: t("scopesNote").replace(/<code>/g, '<code class="text-accent-ink">') }} />
                                                </div>

                                                <div className="md:col-span-2 pt-4">
                                                    <button onClick={handleStripeStep} disabled={saving || !stripeAccountId.trim() || !restrictedKey.trim()} className="w-full py-5 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all duration-500 transform active:scale-95 shadow-xl bg-fg text-surface hover:bg-accent hover:text-fg disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed">
                                                ...
                                                <AnimatePresence>
                                                    {showManualFallback && (
                                                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="md:col-span-2 overflow-hidden">
                                                            <div className="mt-4 p-6 rounded-2xl bg-soon/5 border border-soon/20 space-y-4">
                                                                ...
                                                                <div className="space-y-2">
                                                                    ...
                                                                    <div className="flex gap-3">
                                                                        <input type="text" readOnly value={WEBHOOK_URL} className="flex-1 bg-surface-2/50 border border-hairline rounded-2xl px-5 py-3 text-xs font-mono text-accent-ink outline-none" onClick={(e) => (e.target as HTMLInputElement).select()} />
                                                                        <button onClick={copyWebhookUrl} className="px-4 py-3 rounded-2xl bg-accent/10 text-accent-ink border border-accent/20 hover:bg-accent/20 transition-all font-black text-[10px] uppercase tracking-widest flex items-center gap-2">
                                                                            {copied ? <><Check className="w-3 h-3" />{t("copied")}</> : <><Copy className="w-3 h-3" />{t("copy")}</>}
                                                                        </button>
                                                                    </div>
                                                                    <p className="text-[10px] text-fg-40 ml-1 mt-1" dangerouslySetInnerHTML={{ __html: t("eventsToSelect", { events: RECOMMENDED_EVENTS.join(", ") }).replace(/<code>/g, '<code class="text-accent-ink">') }} />
                                                                </div>
                                                                <div className="space-y-2">
                                                                    <label className="text-[10px] text-fg-40 font-black uppercase tracking-[0.2em] ml-1">{t("signingSecret")}</label>
                                                                    <input type="password" value={webhookSecret} onChange={(e) => setWebhookSecret(e.target.value)} placeholder={t("signingSecretPlaceholder")} className="w-full bg-surface-2/50 border border-hairline rounded-2xl px-5 py-3 text-xs font-medium focus:ring-2 focus:ring-soon/20 focus:border-soon outline-none transition-all placeholder:text-fg-40" />
                                                                </div>
                                                                <button onClick={handleManualSecret} disabled={saving || !webhookSecret.trim()} className="w-full py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 bg-soon/10 text-soon border border-soon/20 hover:bg-soon/20 transition-all disabled:opacity-30 disabled:cursor-not-allowed">
                                        ...
                                        {s.kind === "ix" && (
                                            <div className="grid md:grid-cols-2 gap-8">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <input type="text" value={ixSequenceName} onChange={(e) => setIxSequenceName(e.target.value)} placeholder={t("fieldSeqPlaceholder")} className="w-full bg-surface-2/50 border border-hairline rounded-2xl px-5 py-4 text-sm font-medium focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none transition-all placeholder:text-fg-40" />
                                                </div>
                                                ...
                                                <div className="glass p-6 rounded-2xl flex items-center justify-between border-hairline">
                                                    ...
                                                    <button onClick={() => setVatIncluded(!vatIncluded)} className={cn("w-12 h-6 rounded-full transition-all duration-500 relative ring-1 ring-inset ring-sunken", vatIncluded ? "bg-accent-hot" : "bg-track-off")}><div className={cn("absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-500", vatIncluded ? "left-7" : "left-1")} /></button>
                                                </div>
                                                <div className="glass p-6 rounded-2xl flex items-center justify-between border-hairline">
                                                    <div>
                                                            ...
                                                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="pt-4 border-t border-hairline space-y-4">
                                                                ...
                                                                {!([0, 11.5, 16.5, 21.5, 25].includes(ixRetention)) && (
                                                                    <div className="flex items-center justify-between gap-4 pt-2 border-t border-hairline">
                                                                        ...
                                                                        <div className="w-32"><input type="number" min={0} max={99.99} step={0.01} value={ixRetention} onChange={(e) => { const v = parseFloat(e.target.value); setIxRetention(Number.isFinite(v) ? Math.max(0, Math.min(99.99, v)) : 0); }} className="w-full bg-surface-2/50 border border-hairline rounded-xl px-4 py-2 text-sm font-bold text-center focus:ring-2 focus:ring-destructive/20 outline-none" /></div>
                                                                    </div>
                                                                )}
                                                            </motion.div>
                                                        )}
                                        ...
                                        {s.kind === "activate" && (
                                            <div className="space-y-8">
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                    ...
                                                    <div className="flex items-center gap-3 px-5 py-4 rounded-2xl border bg-accent-hot/5 border-accent-hot/20">
                                                        <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 bg-accent-hot/10"><Webhook className="w-4 h-4 text-accent-hot" /></div>
                                                    ...
                                                    <div className={cn("flex items-center gap-3 px-5 py-4 rounded-2xl border", ixAuthorized ? "bg-accent-hot/5 border-accent-hot/20" : "bg-soon/5 border-soon/20")}>
                                                        <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center shrink-0", ixAuthorized ? "bg-accent-hot/10" : "bg-soon/10")}><ClipboardList className={cn("w-4 h-4", ixAuthorized ? "text-accent-hot" : "text-soon")} /></div>
                                                ...
                                                <button onClick={handleActivate} disabled={saving || !hasWebhookSaved || !ixAuthorized} className="w-full py-5 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all duration-500 transform active:scale-95 shadow-xl bg-fg text-surface hover:bg-accent-hot hover:text-surface disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, ease: "easeOut" }} className="rounded-[2.5rem] p-1 shadow-2xl bg-accent-hot/10">
                        <div className="bg-surface rounded-[2.3rem] p-6 sm:p-10 flex flex-col gap-8 border border-veil">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-veil pt-8">
                                <div className="flex items-center gap-3 px-5 py-4 rounded-2xl border bg-accent-hot/5 border-accent-hot/20"><div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 bg-accent-hot/10"><CreditCard className="w-4 h-4 text-accent-hot" /></div><div><p className="text-[10px] font-black uppercase tracking-wider text-fg-40">{t("stripeLabel")}</p><p className="text-xs font-bold text-accent-hot">{t("active")}</p></div><Check className="w-4 h-4 text-accent-hot ml-auto" /></div>
                                <div className="flex items-center gap-3 px-5 py-4 rounded-2xl border bg-accent-hot/5 border-accent-hot/20"><div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 bg-accent-hot/10"><Webhook className="w-4 h-4 text-accent-hot" /></div><div><p className="text-[10px] font-black uppercase tracking-wider text-fg-40">{t("webhookLabel")}</p><p className="text-xs font-bold text-accent-hot">{t("registered")}</p></div><Check className="w-4 h-4 text-accent-hot ml-auto" /></div>
                                <div className="flex items-center gap-3 px-5 py-4 rounded-2xl border bg-accent-hot/5 border-accent-hot/20"><div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 bg-accent-hot/10"><ClipboardList className="w-4 h-4 text-accent-hot" /></div><div><p className="text-[10px] font-black uppercase tracking-wider text-fg-40">{t("ixLabel")}</p><p className="text-xs font-bold text-accent-hot">{t("statusAuthorized")}</p></div><Check className="w-4 h-4 text-accent-hot ml-auto" /></div>
                            </div>
                        </div>
                    </motion.div>
                )}
