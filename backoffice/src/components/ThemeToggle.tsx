...
export function ThemeToggle() {
  ...
  // Re-asserted on every navigation, not only on mount. Changing the language
  // re-renders <html> from the server, which carries no `data-theme`, and the
  // bootstrap script only runs on a full page load — so without this the
  // language toggle could drag the skin back to the default.
  React.useEffect(() => {
    ...
    const current: Theme =
      stored === "night" || stored === "day" ? stored : DEFAULT_THEME;
